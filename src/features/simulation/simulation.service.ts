...
export class SimulationService {
  ...
  async simulateDay() {
    ...
    for (const advertiser of advertisers) {
      ...
      const latestBudget = await this.budgetsRepository.getLatestBudget(advertiser.id);
      ...
      if (latestBudget) {
        ...
        const existingBudget = await this.budgetsRepository.getBudgetForDay(advertiser.id, newDayString);

        if (!existingBudget) {
          await this.budgetsRepository.createBudgetForDay(advertiser.id, newDayString);
        }

        await this.rescheduleDeferredCampaigns(advertiser.id, newDayString, newBalance);
      } else {
        const today = new Date().toISOString().slice(0, 10);
        console.log(`No previous budget for ${advertiser.id}, creating one for today: ${today}`);
        await this.budgetsRepository.createBudgetForDay(advertiser.id, today);
        await this.rescheduleDeferredCampaigns(advertiser.id, today, newBalance);
      }
    }
  ...
  private async rescheduleDeferredCampaigns(advertiserId: string, currentDay: string, currentBalance: number) {
    const deferredCampaigns = await this.campaignsRepository.getDeferredCampaigns(advertiserId);
    ...
    const newDayBudget = await this.budgetsRepository.getBudgetForDay(advertiserId, currentDay);
